...
class GaussianRenderer(nn.Module):
    ...
    def compute_gaussian_values(
        ...
        det = torch.linalg.det(covs2D)
        inv_cov = torch.linalg.inv(covs2D)

        # Compute quadratic form: dx^T Σ^(-1) dx
        quad = torch.einsum('nhwi,nij,nhwj->nhw', dx, inv_cov, dx)

        # Compute Gaussian values: 1/(2π√|Σ|) * exp(-1/2 * quad)
        gaussian = (1.0 / (2.0 * np.pi * torch.sqrt(det + 1e-6))).unsqueeze(-1).unsqueeze(-1) * torch.exp(-0.5 * quad)
    ...
    def forward(
        ...
        # 7. Compute weights
        T = torch.cumprod(1.0 - alphas + 1e-6, dim=0)
        weights = T * alphas
